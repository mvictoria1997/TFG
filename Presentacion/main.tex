...
\begin{frame}[c]{Comparativa computación cuántica y clásica}
	\begin{figure}
		...
		\caption{Comparativa de cómputo de de un ordenador cuántico y clásico}
	\end{figure}
\end{frame}
...
\begin{frame}[c]{Descripción}
	Una cadena de bloques es un sistema de almacenamiento de información dividido en bloques de datos enlazados mediante el \textit{hash}.
	\begin{figure}
		\centering
		\includegraphics[height=4cm]{arbol_merkle.png}
		\caption{Estructura árbol de Merkle}
	\end{figure}
\end{frame}

%Algunas aplicaciones a destacar son en el área financiera para almacenar transacciones de criptomonedas, en cadenas de suministro de restaurante como Fogo de Chao, centros de salud, para almacenar historiales médicos o para la firma de documentos en notarías.
...
\begin{frame}[c]{Bloques del diseño}
	...
	\begin{exampleblock}{Base de datos}
		Almacenar y servir datos de las transacciones y bloques.
	\end{exampleblock}

	\begin{exampleblock}{ARK Desktop Wallet}
		Interfaz para la realización de transacciones.
	\end{exampleblock}
	
	\begin{exampleblock}{Explorer ARK}
...
\begin{frame}[c]{Posibles configuraciones de los bloques}
	\begin{columns}[onlytextwidth]
		...
		\begin{figure}
			\centering
			\includegraphics[height=3.5cm]{diagrama_bloquesARK.png}
			\caption{Diagrama de bloques prototipo}
		\end{figure}
		\column{0.5\textwidth}
		\begin{figure}
			\centering
			\includegraphics[height=3.5cm]{diagrama_bloquesARK3.png}
			\caption{Diagrama de bloques ejemplo}
		\end{figure}
	\end{columns}
\end{frame}
